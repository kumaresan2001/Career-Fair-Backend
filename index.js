...
app.get("/", function (req, res) {
  res.send("<h1>E-commerce Product Listing Project...</h1>");
});
...
app.post("/Products", async (req, res) => {
  try {
    const connection = await mongoclient.connect();
    const db = connection.db(DB);
    const Products = await db.collection("product").insertOne(req.body);
    res.json(Products);
    await connection.close();
  } catch (error) {
    console.log(error);
...
app.get("/Products", async (req, res) => {
  try {
    const connection = await mongoclient.connect();
    const db = connection.db(DB);
    const Products = await db.collection("product").find(req.query).toArray();
    res.json(Products);
    await connection.close();
  } catch (error) {
    console.log(error);
